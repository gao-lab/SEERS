...
combined_data
# nuc and cyt scores:
if (all(c("nuc.score", "cyt.score") %in% names(combined_data))) {
  nuc_scores <- combined_data[["nuc.score"]]
  cyt_scores <- combined_data[["cyt.score"]]
} else if (all(c("nuc", "cyt") %in% names(combined_data))) {
  nuc_scores <- log2(combined_data[["nuc"]] + 1)
  cyt_scores <- log2(combined_data[["cyt"]] + 1)
} else {
  stop("combined_data must contain nuc.score/cyt.score or nuc/cyt columns")
}
# expression score:
